...
final class InventoryTests: XCTestCase {
    ...
    func testEndToEndInventoryFlow() async throws {
      ...
      try await app.test(.POST, "/api/inventory/inventory/inbound", beforeRequest: { req in
        ...
        let payload = InboundRequestDTO(
          productID: product.id,
          locationID: locA.id,
          batchID: batchID,          // або nil якщо немає
          qty: 100,
        ...
        try req.content.encode(payload)
          ...
          XCTAssertEqual(s.available, 100)
      })
      ...
      try await app.test(.POST, "/api/inventory/inventory/outbound", beforeRequest: { req in
          req.headers.bearerAuthorization = .init(token: userToken)
        let payload = OutboundRequestDTO(
          productID: product.id,
          locationID: locA.id,
          batchID: batchID,
          qty: 50,                   // або 999 для negative
          reason: "shipment #5001",  // або "neg"
          reference: "SO-5001"       // або "NEG-1"
        )
        try req.content.encode(payload)
      ...
      try await app.test(.POST, "/api/inventory/inventory/transfer", beforeRequest: { req in
          req.headers.bearerAuthorization = .init(token: userToken)
        let req8 = TransferRequestDTO(productID: product.id, fromLocationID: locA.id, toLocationID: locB.id, batchID: batchID, qty: 40, reason: "rebalancing", reference: "TR-0001")
          try req.content.encode(req8)
      }, afterResponse: { res in
          XCTAssertEqual(res.status, .ok)
          let pair = try res.content.decode([StockLevelResponseDTO].self)
      ...
      try await app.test(.POST, "/api/inventory/inventory/set-onhand", beforeRequest: { req in
          req.headers.bearerAuthorization = .init(token: userToken)
        let req92 = SetOnHandRequestDTO(productID: product.id, locationID: locB.id, batchID: batchID, newValue: 35, reason: "inventory count", reference: "INV-CNT")
          try req.content.encode(req92)
      }, afterResponse: { res in
      ...
      try await app.test(.POST, "/api/inventory/inventory/set-onhand", beforeRequest: { req in
          req.headers.bearerAuthorization = .init(token: adminToken)
        let req93 = SetOnHandRequestDTO(productID: product.id, locationID: locB.id, batchID: batchID, newValue: 35, reason: "inventory count", reference: "INV-CNT")
          try req.content.encode(req93)
      }, afterResponse: { res in
          XCTAssertEqual(res.status, .ok)
          let s = try res.content.decode(StockLevelResponseDTO.self)
      ...
      try await app.test(.POST, "/api/inventory/inventory/outbound", beforeRequest: { req in
          req.headers.bearerAuthorization = .init(token: userToken)
        let req10 = OutboundRequestDTO(productID: product.id, locationID: locA.id, batchID: batchID, qty: 999, reason: "neg", reference: "NEG-1")
          try req.content.encode(req10)
      }, afterResponse: { res in
      ...
      try await app.test(.POST, "/api/inventory/inventory/reserve", beforeRequest: { req in
          req.headers.bearerAuthorization = .init(token: userToken)
        let payload = ReserveRequestDTO(productID: product.id, locationID: locA.id, batchID: batchID, qty: 0, reason: "neg", reference: "NEG-2")
          try req.content.encode(payload)
      }, afterResponse: { res in
