...
struct InventoryController: RouteCollection {
  func boot(routes: any RoutesBuilder) throws {
    ...
    group.get("levels", use: listLevels)
    group.get("movements", use: listMovements)
  ...
  func inbound(req: Request) async throws -> StockLevelResponseDTO {
    ...
    let s = try await req.appServices.inventory.inbound(productID: dto.productID, to: dto.locationID, batchID: dto.batchID, qty: dto.qty, reason: dto.reason, reference: dto.reference, performedBy: userID, on: req.db)
    return .init(s)
  }
  ...
  func outbound(req: Request) async throws -> StockLevelResponseDTO {
    ...
    let s = try await req.appServices.inventory.outbound(productID: dto.productID, from: dto.locationID, batchID: dto.batchID, qty: dto.qty, reason: dto.reason, reference: dto.reference, performedBy: userID, on: req.db)
    return .init(s)
  }
  ...
  func transfer(req: Request) async throws -> [StockLevelResponseDTO] {
    ...
    let pair = try await req.appServices.inventory.transfer(productID: dto.productID, from: dto.fromLocationID, to: dto.toLocationID, batchID: dto.batchID, qty: dto.qty, reason: dto.reason, reference: dto.reference, performedBy: userID, on: req.db)
  ...
  func reserve(req: Request) async throws -> StockLevelResponseDTO {
    ...
    let s = try await req.appServices.inventory.reserve(productID: dto.productID, at: dto.locationID, batchID: dto.batchID, qty: dto.qty, reason: dto.reason, reference: dto.reference, performedBy: userID, on: req.db)
    return .init(s)
  }
  ...
  func unreserve(req: Request) async throws -> StockLevelResponseDTO {
    ...
    let s = try await req.appServices.inventory.unreserve(productID: dto.productID, at: dto.locationID, batchID: dto.batchID, qty: dto.qty, reason: dto.reason, reference: dto.reference, performedBy: userID, on: req.db)
    return .init(s)
  }
  ...
  func setOnHand(req: Request) async throws -> StockLevelResponseDTO {
    ...
    let s = try await req.appServices.inventory.setOnHand(productID: dto.productID, at: dto.locationID, batchID: dto.batchID, to: dto.newValue, reason: dto.reason, reference: dto.reference, performedBy: user.id, on: req.db)
    return .init(s)
  }
}
